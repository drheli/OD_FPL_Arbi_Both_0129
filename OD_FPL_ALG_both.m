...
% if j > delta
% p = p(j-delta)
function [CAd_plus,CAd_minus,CAq_plus_o,CAq_minus_o,CAq_plus_sel, CAq_minus_sel,CAd_plus_sel, CAd_minus_sel, shift_to_int_plus,shift_to_int_minus,v_frac_plus,v_frac_minus,compare_frac,w_frac_plus,w_frac_minus,shift_out_plus,shift_out_minus,count_one_plus,count_one_minus,q_plus,q_minus,v_int_plus,v_int_minus,w_int_plus,w_int_minus] = OD_FPL_ALG_both(x_in_plus, x_in_minus, d_in_plus, d_in_minus, k, n_r, u_r)
% initialization step
% time_step: j
unrolling = 8;
delta = 4;
...
if isempty(CAq_plus) || isempty(CAq_minus) 
...
%for j = 1:64
        ...
        for i = n_r:-1:0
            ...
            if (n_r==0)&&(u_r<=delta)
                ...
                q_plus =0; q_minus=0;
                CAq_plus_o = zeros(1,unrolling); CAq_minus_o = zeros(1,unrolling);
                %CAq_plus = zeros(1,unrolling); CAq_minus = zeros(1,unrolling);
            else
                % CA[dj+1]
                [CAd_plus,CAd_minus] = CA_gen_d(d_plus_rev,d_minus_rev,i,i,u_r,k,enable);
