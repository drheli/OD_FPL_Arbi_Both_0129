...
%for j = 1:18
        ...
        for i = n_r:-1:0
            ...
            if (n_r==0)&&(u_r<=delta)
                ...
                u_r_q = unrolling * n_r + u_r - delta;
                [CAq_plus,CAq_minus] = CA_gen_q(q_plus_rev,q_minus_rev,i,i,u_r_q,k,enable);
                ...
                CAw_frac_plus = w_plus_wr_frac(pairing(i,k),:);
                CAw_frac_minus = w_minus_wr_frac(pairing(i,k),:);
                % in v_frac_div, if rd_addr == 0, begin to deliver updated updated shift_to_int
                % in OM, it shown in the Top Module
                % Exactly, when rd_addr == 0, generate q.
                [shift_to_int_plus,shift_to_int_minus,v_frac_plus,v_frac_minus,compare_frac] = v_frac_div(CAq_plus_sel,CAq_minus_sel,CAw_frac_plus,CAw_frac_minus,x_plus_rev,x_minus_rev,  i,n_r);
